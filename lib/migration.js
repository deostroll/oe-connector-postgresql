...
function mixinMigration(PostgreSQL) {
  ...
  PostgreSQL.prototype.buildColumnDefinition = function (model, propName) {
    ...
    if(typeof prop.postgres === 'object' && 'sequence' in prop.postgres && prop.postgres.sequence.type === 'standard') {
      let seqInfo = Object.assign({}, SEQ_STANDARD, prop.postgres.sequence);
    ...
    else if(typeof prop.postgres === 'object' && 'sequence' in prop.postgres && prop.postgres.sequence.type === 'simple') {
      let seqName = prop.postgres.sequence.name;
  ...
  PostgreSQL.prototype.createTable = function (model, cb) {
    ...
    var sequenceSupportedProperties = Object.keys(properties).reduce((carrier, key) => {
      ...
      if (typeof currentEntry === 'object' && 'postgres' in currentEntry && 'sequence' in currentEntry.postgres) {

...
PostgreSQL.prototype._buildFieldsForKeys = function (model, data, keys, excludeIds) {
  ...
  var seqFields = Object.keys(props).reduce((carrier, key) => {
    ...
    if(propDef.postgres && propDef.postgres.sequence) {
  ...
  if(seqFields.length) {
    ...
    if(def.postgres.sequence.type === 'complex') {      
...
PostgreSQL.prototype.toColumnValue = function (prop, val) {
  if (val == null) {
    ...
    if(prop.postgres && prop.postgres.sequence && prop.postgres.sequence.type === 'complex') {
      let { length, prefix, name } = prop.postgres.sequence;

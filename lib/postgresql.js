...
PostgreSQL.prototype.updateAndCall = function (fn, args) {
  ...
  if (self._modelUpdateIds[m] !== updateId) {
...
PostgreSQL.prototype._buildFieldsForKeys = function (model, data, keys, excludeIds) {
  ...
  var seqFields = Object.keys(props).reduce((carrier, key) => {
    let propDef = props[key];
    if(propDef.postgresql && propDef.postgresql.sequence) {
      carrier.push([key, propDef]);
    }
    return carrier;
  }, []);
  var fields = {
  ...
  if(seqFields.length) {
    let [[propName, def]] = seqFields;
    if(def.postgresql.sequence.type === 'complex') {      
      fields.names.push(this.columnEscaped(model, propName));
      fields.columnValues.push(this.toColumnValue(def, null));
      fields.properties.push(def);
    }
  }
  for (var i = 0, n = keys.length; i < n; i++) {
    ...
    if (v !== undefined) {
      ...
      if (Array.isArray(p.type)) {
        ...
        if (flag && (p.type[0].name === 'String' || p.type[0].name === 'Number' || p.type[0].name === 'Date')) {
        ...
        // Atul : taking care of loopback3 and loopback2 both
          ...
          //console.log('came');
          if(typeof paramdata.params[0] === 'string'){
              paramdata.params[0] = JSON.parse(paramdata.params[0]);
          }
          // else do nothing. - paramdata.params[0] = JSON.parse(paramdata.params[0]);
        }
      }
...
PostgreSQL.prototype.buildInsert = function (model, data, options) {
  //stringifying JSON, Object, Array
  //to support array, array of object data type (counter measure for overriden _forDB of dao)
  var res = {};
  for (var propName in data) {
   var type = typeof(data[propName]);
...
PostgreSQL.prototype.toColumnValue = function (prop, val) {
  if (val == null) {
    ...
    if(prop.postgresql && prop.postgresql.sequence && prop.postgresql.sequence.type === 'complex') {
      let { length, prefix, name } = prop.postgresql.sequence;
      let strLength = length - prefix.length;
      return new ParameterizedSQL(`'${prefix}' || LPAD(CAST(nextval('${name}') AS VARCHAR), ${strLength}, '0')`);
    }
...
PostgreSQL.prototype.fromRow = PostgreSQL.prototype.fromDatabase =
  function (model, rowData) {
    ...
    for (var p in props) {
      ...
      if (columnValue !== undefined && (data[p] || data[p] === false || data[p] === 0 || data[p] === '')) {
        data[p] = columnValue;
      } else if (columnValue !== undefined && typeof data[p] === 'undefined') {
        data[p] = columnValue;
